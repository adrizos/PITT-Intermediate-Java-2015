...
public class Assig5b
{
	...
	private JMenuItem endProgram, saveScene, saveAs,open, newScene;  	// 4 menu items will be used in this
	...
	private String fileToOpen; //filename in which to open
	...
	private class MenuHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				ArrayList <MyPoly> tempPolyList = new ArrayList<>();
				MyPoly temp;
				boolean skipped = false;
				//tempPolyList.add(shapeList.get(selindex)); //add the selected shape to the beginning
				temp = shapeList.get(selindex);
				tempPolyList = shapeList;
				tempPolyList.remove(selindex);
				tempPolyList.add(0, temp);
				shapeList = tempPolyList; //assign the new order of the list to shapeList
				drawPanel.repaint();
                modifiedSinceSave = true;
			}
			...
			{
                int selectedOption;
                selectedOption = JOptionPane.showConfirmDialog(null,"Save?","Choose",JOptionPane.YES_NO_OPTION);
                if (selectedOption == JOptionPane.YES_OPTION)
                {
                    ...
                    saveImages();
                    modifiedSinceSave = false; //reset notification
                }
            }
			...
			{
                //if modified since last save NEED TO ADDRESS THIS
                if (modifiedSinceSave == true)
                {
                    int selectedOption;
                    selectedOption = JOptionPane.showConfirmDialog(null,"Save?","Choose",JOptionPane.YES_NO_OPTION);
                    ...
                    if (selectedOption == JOptionPane.YES_OPTION)
                    {
                        if (currFile == null) // if changes have been made and not saved, save first
                        {
                            currFile = JOptionPane.showInputDialog(theFrame,"Enter file name to save first");
                        }
                        saveImages();
                        System.exit(0);
                    }
			...
			else if (e.getSource() == open) // open new scene when clicked
			{
				//if modified since last save NEED TO ADDRESS THIS
                if (modifiedSinceSave == true)
                {
				...
				//enter file to open
				fileToOpen = JOptionPane.showInputDialog(theFrame,"Enter file name to open");

				//clear scene and call loadFile()
				if (shapeList.size() > 0)
					clearAll();
				loadFile();
			}
            else if (e.getSource() == newScene) // needs modified clause
            {
                //if modified since last save NEED TO ADDRESS THIS
                if (modifiedSinceSave == true)
                {
	...
	public void loadFile()
	{
		...
		try
		{
			...
			while (textScan.hasNextLine()) //
			{
				...
				while(textScan.hasNext())
				{
					...
					String [] removeRGB = textScan.nextLine().split("[|]");
					//note:removeRGB[1] = color profile
					String [] RGBsingles = removeRGB[1].split(","); //single string values of each R/G/B
					int [] RGBsinglesInt = new int [RGBsingles.length];
					for (int d=0; d < RGBsingles.length; d++)
					{
						RGBsinglesInt[d] = Integer.parseInt(RGBsingles[d]);
					}	//loop converts RGB string values to int values
					//convert int RGB values to Color object value that can pass to new polygon constructor
					Color tempColor = new Color (RGBsinglesInt[0],RGBsinglesInt[1],RGBsinglesInt[2]);

					//handle coordinates
					String [] tempStr = removeRGB[0].split(":"); //x and y values as strings with commas ex. "x,y"

					int [] xValues = new int[tempStr.length];
					int [] yValues = new int[tempStr.length];
					for (int p =0; p <tempStr.length; p++)
					{
						String [] temp2 = tempStr[p].split(",");
						xValues[p] = (Integer.parseInt(temp2[0]));
						yValues[p] = (Integer.parseInt(temp2[1]));
					} //loop splits each "x,y" to corresponding x and y int arrays
					//create new polygon
					MyPoly tempPoly = new MyPoly(xValues,yValues,tempStr.length,tempColor);
					//add tempPoly to shapelist array
					addshape(tempPoly);
				} //end of while loop 1 iteration for each shape
		...
		{
			JOptionPane.showMessageDialog(theFrame, "I/O Problem - File not Loaded");
		}
	} //end of load in data method
	...
	private void addshape(MyPoly newshape)
	{
		...
		drawPanel.repaint();
        modifiedSinceSave = true; // should this be here?
	}
	...
	private class ShapePanel extends JPanel
	{
		...
		private class MyMouser extends MouseAdapter
		{
            ...
            /*public void mouseMoved(MouseEvent e) // this method was part of me trying to impliment the fill circles properly
            {
                ...
                if (mode == MODIFY)
                {
                    ...
                    if (e.getX() < x1+8 && e.getY() < y1+8)
						shapeList.get(selindex).draw();
